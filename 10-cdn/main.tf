resource "aws_cloudfront_distribution" "devops1" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.devops1.domain_name
        zone_id = aws_cloudfront_distribution.devops1.hosted_zone_id
      }
    }
  ]
}